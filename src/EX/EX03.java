...
public class EX03 {
    ...
    public static String findMostFrequentlyOccurringLetter(String text) {
        int lausePikkus = text.length();
        int EelmineTaht = 0;
        char w;
        String esimeneTaht = "";
        String taht = "";
        for (int x = 0; text.length() > x; x++) {
            esimeneTaht = String.valueOf(text.charAt(x));
            String a = text.replace(esimeneTaht, "");
            int pikkus = a.length();
            int taheArv = lausePikkus - pikkus;
            if (taheArv > EelmineTaht){
                EelmineTaht = taheArv;
                taht = esimeneTaht;
    ...
    public static String minimizeText(String text) {
        String MiniText = "";
        String fag = findMostFrequentlyOccurringLetter(text);
        int len = text.length();
        for (int x = 0; x < len; x++) {
